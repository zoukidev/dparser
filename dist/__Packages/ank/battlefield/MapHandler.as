class ank.battlefield.MapHandler
{
   ...
   function initializeCell(nCellNum, nPermanentLevel)
   {
      ...
      var _loc6_ = _loc4_.originalsCellsBackup.getItemAt(String(nCellNum));
      if(_loc6_ == undefined)
      {
         ank.utils.Logger.err("La case est déjà dans son état init");
         return undefined;
      }
      if(_loc5_[nCellNum].nPermanentLevel <= nPermanentLevel)
      {
         _loc5_[nCellNum] = _loc6_;
         this.build(_loc4_,nCellNum);
         _loc4_.originalsCellsBackup.removeItemAt(String(nCellNum));
      }
   }
